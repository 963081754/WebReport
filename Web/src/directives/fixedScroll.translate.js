  ...
  const onScroll = function (el, { top = [], left = [], right= [] } = { top: [], left: [], right: [] }) {
    ...
    if (top.join(',') !== '') {
      ...
      elList.forEach(ele => {
        ...
        ele.style.transform = `translate3d(0px,${axis.y}px,10px)`
      })
    }
    if (left.join(',') !== '') {
      const elList = el.querySelectorAll(left.join(','))
      elList.forEach(ele => {
        ele.style.transform = `translate3d(${axis.x}px,0px,-10px)`
      })
      // debugger
      // const t = elList[0].querySelector('th')
      // if(t)
      //   t.style.transform.translateZ(-1)
    }
    if (right.join(',') !== '') {
 ...
 */
  Vue.directive('fixedScroll', {
    inserted: function (el, binding) { // binding.value ={ top: [selector,selector……],left: [selector,selector……]}
      let hasSet = false
