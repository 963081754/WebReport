    ...
    <div class="manage reportManage" v-onresize="{callback:onresize,dir:'w'}">
        ...
        <div class="treeTable" :class="{loading}">
            <div class="treeHeader">
                <div class="name"><label>名 称</label></div>
                <div class="roles" :style="{width:`${roleColumnWidth}px`}">
                    角色权限
                    （
                    <select  v-model.number="roleShowMode">
                ...
                <template v-slot:default="{item}">
                    <div class="treeTr" :class="{from:item.model && item.model.from_id, hide:item.model && item.model.hide,disabled:item.model && !item.model.enabled}">
                        <template v-if="item.model">
                            ...
                            <div class="toff" v-if="roleShowMode!==1" :title="getNamesByKeys('role',item.model.role_ids)" :style="{width:`${roleColumnWidth}px`}">{{getRoleGroupName(item.model.role_ids)}}</div>
                            <div class="toff roles" v-else :title="getNamesByKeys('role',item.model.role_ids)" :style="{width:`${roleColumnWidth}px`}">
                                <template v-if="item.model.role_ids.length">
                                    <label class="toff" v-for="(role) in roles" :class="{active:item.model.role_ids.includes(role.id)}" :key="role.id">{{role.name}}</label>
                                </template>
                                <template v-else>
                                    <label class="unlimited">无限制</label>
                                </template>
                            </div>
                            <!-- <div class="toff" :title="getNamesByKeys('user',[item.model.userKeys,item.model.sqlKeys])"></div> -->
                            <div class="toff userBands">
                                <label v-for="item in getUserBands(item.model)" :key="item">{{item}}</label>
                            </div>
                            ...
                            <div class="opt">
                                ...
                                <!-- <i class="more fa fa-angle-double-down"></i> -->
                                <i class="more">…</i>
                                <div class="moreBox">
                                    <i :class="`fa fa-${item.model.star || 'file'} starIcon`" @click.stop="openIconSelector(item.model)" hint2="选择图标"></i>
                                    <r-toggle class="show" :checked="!item.model.hide" :iconMode="1" @change="hide(item.model)" :hint2="item.model.hide?'显示':'隐藏'"/>
...
export default {
    ...
    computed:{
        ...
        getNamesByKeys(){
            return (type,value)=>{
                switch(type){
                    ...
                    //     return value.length > 0 ? value.join(',') : '' //userKeys + sqlKeys
        ...
        roles(){
            const obj = {}
            this.list.map(t=>t.role_ids).filter(t=>t.length).forEach(ids=>ids.forEach(id=>{
                obj[id] = obj[id] ? obj[id] + 1 : 1
            }))
            const ary = Object.entries(obj).sort((a,b)=>b[1] - a[1])
            // console.log(ary)
            return ary.map(item=>this.getters.roles.find(t=>t.id === item[0])).filter(t=>t)
        }
    },
    watch:{
        refresh(){
    ...
    methods:{
        ...
        async updateSeq({item1,item2, type, index}){
            if(item1.model){ // item1是report
                // item1.model.pid = xxxxx // 冻结了，不能改；这个页面也不用显示，就不改。
                let category_id
                if(!item2.model){
                    category_id = item2.id
                    const reports = item2.children.filter(t=>t.model)
                    if(reports.length > 0){
                        item2 = reports[0]
                    }else{
                        item2 = null
                    ...
                    category_id = item2.model.category_id
                }
                await this.reportApi.sort({category_id,id1:item1.model.id,id2:item2 && item2.model.id,type})  // 报表排序
            }else{ // item1是catetory
                this._categorySort = true
                const model = {...this.getters.categorys.find(t=>t.id === item1.id),pid:item1.pid,seq:index}
        ...
        getRoleGroupName(role_ids){
            if(this.roleShowMode === 3) return undefined
        ...
        getUserBands(report){
            const bands = report.userKeys.map(t=>t.ufield).concat((report.sqlKeys || []).map(t=>`@${t}`))
            return bands
        },
        onresize(el){
            this.roleColumnWidth = Math.max(roleColumnWidth,roleColumnWidth + (el.clientWidth - 1300)*(9/10))
            ...
            > div{
                ...
                &.opt{
                    ...
                    > .moreBox{
                        ...
                        border-radius: 2px;
                        box-shadow: -1px 1px 2px 0px #987;
                        &:hover{
                            display: block;
                        }
                    }
